...
func main() {
	...
	//TODO: multiple files
	filepath := "inputs/2016-readings.xml"
	//filePathXML := "inputs/2016-readings.xml"

	fileReader := readers.NewFileReader()

	readings, err := fileReader.ReadReadings(filepath)
	...
	suspiciousReadingsDetector := detector.NewReadingDetector()
	suspiciousReadings, err := suspiciousReadingsDetector.DetectSuspiciousReadings(readings)
	fmt.Println(suspiciousReadings)

	suspiciousReadingsWriter := writer.NewReadingWriter()
	filename := "suspicious_readings.csv"
	err = suspiciousReadingsWriter.WriteReadingsToFile(filename, suspiciousReadings)
	fmt.Println(filename)
	// readingsXML, err := fileReader.ReadReadings(filePathXML)
	// if err != nil {
